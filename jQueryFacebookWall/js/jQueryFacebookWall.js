    ...
    Plugin.prototype = {
        ...
        init: function() {
				...
				if($target.hasClass('album-cover-photo-wrapper')){
					...
					if(albumData.photos==undefined){
						...
						$.getJSON(albumDataUrl,function(photoData){
							...
							_this._popLightBox(content, _this);	
						});
					}else{
						$.extend(albumData, {
							templatePath:_this.options.installDirectory
			 ...
			 */
			  ...
			  */
			 this.$element.on('click.jQueryFacebookWall','.comments-view-more', function(event){ _this._showMoreComments( event, _this ) } );
		      ...
		      */
			 this.$element.on('click.jQueryFacebookWall','.photo-gallery-control-left',{increment:-1}, function(event){_this._photoGalleryPaginate(event,_this)} );
			 this.$element.on('click.jQueryFacebookWall','.photo-gallery-control-right',{increment:1}, function(event){_this._photoGalleryPaginate(event,_this)} );
		...
		*/
		_popLightBox: function(content, _this){	
			...
			$overlay.next().fadeIn(function(){
				...
				_this._updateImageDimension($lightbox.find('.jfw-lightbox-content .photo-gallery-photo.show'), $lightbox.find('.jfw-lightbox-content .photo-gallery-photos'))
			});
		},
		...
		_fbLogin: function() {
			...
			FB.login(function(response) {
				if (response.authResponse) {
					// connected
					if(_this.options.debug){console.log('_fbLogin: user authenticated:'); console.log(response);}
				} 
				else {
					// cancelled
				}
